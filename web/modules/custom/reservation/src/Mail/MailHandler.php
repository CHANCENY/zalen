...
use Drupal\reservation\Mail\OrderMailManager;
...
final class MailHandler {
  ...
  public function sendMail(string $to, TranslatableMarkup $subject, array $body, array $params = []): bool {
    $default_params = [
      ...
      // The body will be rendered in reservation_mail().
      'body' => $body,
    ];
    if (!empty($params['cc'])) {
    ...
    $message = \Drupal::service('reservation.manager_html_mail');
    $message = $message->mail('reservation', $params['id'], $to, $params['langcode'], $params, $params['reply-to']);
