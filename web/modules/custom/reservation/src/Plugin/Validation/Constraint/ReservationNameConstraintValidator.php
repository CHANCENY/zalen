...
/**
 * Validates the ReservationName constraint.
 */
class ReservationNameConstraintValidator extends ConstraintValidator implements ContainerInjectionInterface {
  ...
  /**
   * Constructs a new ReservationNameConstraintValidator.
   *
  ...
  public function validate($entity, Constraint $constraint) {
    ...
    // Do not allow unauthenticated reservation authors to use a name that is
    ...
    // can't validate this without a valid reservationed entity, which will fail
    // the validation elsewhere.
    if ($owner_id === 0 && empty($author_name) && $entity->getReservationedEntity() && $entity->getFieldName() &&
      $this->getAnonymousContactDetailsSetting($entity) === ReservationInterface::ANONYMOUS_MUST_CONTACT) {
  ...
  /**
   * Gets the anonymous contact details setting from the reservation.
   *
   ...
   */
  protected function getAnonymousContactDetailsSetting(ReservationInterface $reservation) {
    return $reservation
      ->getReservationedEntity()
      ->get($reservation->getFieldName())
      ->getFieldDefinition()
