...
/**
 * Field handler to display the number of new reservations.
 *
 * @ingroup views_field_handlers
 *
 * @ViewsField("node_new_reservations")
 */
class NodeNewReservations extends NumericField {
  ...
  /**
   * Constructs a \Drupal\reservation\Plugin\views\field\NodeNewReservations object.
   *
  ...
  public function init(ViewExecutable $view, DisplayPluginBase $display, array &$options = NULL) {
    ...
    $this->additional_fields['reservation_count'] = ['table' => 'reservation_entity_statistics', 'field' => 'reservation_count'];
  }
  ...
  protected function defineOptions() {
    ...
    $options['link_to_reservation'] = ['default' => TRUE];
  ...
  public function buildOptionsForm(&$form, FormStateInterface $form_state) {
    $form['link_to_reservation'] = [
      '#title' => $this->t('Link this field to new reservations'),
      '#description' => $this->t("Enable to override this field's links."),
      '#type' => 'checkbox',
      '#default_value' => $this->options['link_to_reservation'],
    ];
  ...
  public function preRender(&$values) {
    ...
    if ($nids) {
      $result = $this->database->query("SELECT n.nid, COUNT(c.cid) AS num_reservations FROM {node} n INNER JOIN {reservation_field_data} c ON n.nid = c.entity_id AND c.entity_type = 'node' AND c.default_langcode = 1
      ...
      foreach ($result as $node) {
        foreach ($ids[$node->nid] as $id) {
          $values[$id]->{$this->field_alias} = $node->num_reservations;
        }
      }
    }
  }
  ...
  /**
   * Prepares the link to the first new reservation.
   *
   * @param string $data
  ...
  protected function renderLink($data, ResultRow $values) {
    if (!empty($this->options['link_to_reservation']) && $data !== NULL && $data !== '') {
      $node_type = $this->getValue($values, 'type');
      $node = Node::create([
        'nid' => $this->getValue($values, 'nid'),
        'type' => $node_type,
      ]);
      // Because there is no support for selecting a specific reservation field to
      // reference, we arbitrarily use the first such field name we find.
      // @todo Provide a means for selecting the reservation field.
      //   https://www.drupal.org/node/2594201
      $field_map = $this->entityFieldManager->getFieldMapByFieldType('reservation');
      $reservation_field_name = 'reservation';
      foreach ($field_map['node'] as $field_name => $field_data) {
        foreach ($field_data['bundles'] as $bundle_name) {
          if ($node_type == $bundle_name) {
            $reservation_field_name = $field_name;
            break 2;
          }
        }
      }
      $page_number = $this->entityTypeManager->getStorage('reservation')
        ->getNewReservationPageNumber($this->getValue($values, 'reservation_count'), $this->getValue($values), $node, $reservation_field_name);
