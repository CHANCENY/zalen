 ...
 * )
 */
class EntityReservation extends EntityContentBase {
  ...
  /**
   * An array of entity IDs for the 'reservationed entity' keyed by entity type.
   *
   * @var array
   */
  protected $stubReservationedEntityIds;

  /**
   * Builds a reservation entity destination.
   *
  ...
  public function import(Row $row, array $old_destination_id_values = []) {
    if ($row->isStub() && ($state = $this->state->get('reservation.maintain_entity_statistics', 0))) {
      $this->state->set('reservation.maintain_entity_statistics', 0);
    }
    $return = parent::import($row, $old_destination_id_values);
    if ($row->isStub() && $state) {
      $this->state->set('reservation.maintain_entity_statistics', $state);
    }
    return $return;
  }
