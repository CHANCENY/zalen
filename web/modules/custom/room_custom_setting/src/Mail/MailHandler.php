...
use Drupal\room_custom_setting\Mail\OrderMailManager;
...
final class MailHandler {
  ...
  public function sendMail(string $to, TranslatableMarkup $subject, array $body, array $params = []): bool {
    $default_params = [
      ...
      // The body will be rendered in room_custom_setting_mail().
      'body' => $body,
    ];
    if (!empty($params['cc'])) {
    ...
    $message = \Drupal::service('room_custom_setting.manager_html_mail');
    $message = $message->mail('room_custom_setting', $params['id'], $to, $params['langcode'], $params, $params['reply-to']);
