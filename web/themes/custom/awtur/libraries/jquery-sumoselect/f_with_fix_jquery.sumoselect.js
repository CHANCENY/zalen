...
(function (factory) {
  ...
  $.fn.SumoSelect = function (options) {
    ...
    const defaultOptions = {
      placeholder: 'Selecteer',   // Dont change it here.
      ...
      this.sumo = {
        ...
        toggSelAll (c, direct) {
          ...
          if (O.is_multi) {
            // Select all
            if (c) {
              O.E.find('option:not(:checked):not(:disabled)').toArray().forEach(option => {
              ...
              // Unselect all
              O.E.find('option:checked:not(:disabled)').toArray().forEach(option => {
                if (!$(option).data('li').hasClass('hidden')) {
